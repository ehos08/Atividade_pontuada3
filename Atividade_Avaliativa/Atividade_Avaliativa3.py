...
def calcular_plano_saude(necessitados):
    desconto_dependente = 150
    return necessitados * desconto_dependente
cadastro()
if login():
    ...
    vale_refeicao = float(input("Digite o valor do vale refeição oferecido pela empresa: "))

    # Cálculo dos descontos
    desconto_inss = inss(salario_bruto)
    desconto_irrf = irrf(salario_bruto, necessitados)
    desconto_valeRF = vale_refeicao * 0.20
    desconto_planoS = calcular_plano_saude(necessitados)
...
# Soma total dos descontos
    descontos_total = desconto_inss + desconto_irrf + desconto_valeRF + desconto_planoS + desconto_valeT
